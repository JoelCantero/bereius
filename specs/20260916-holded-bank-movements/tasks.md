...
- [ ] T057 Add production-artifact Playwright coverage for signed-out redirect, operator/admin roles, all three locales, filters/full totals, refresh states, manual proposal confirmation, Holded-outage retained rows, one unrelated booking action, banking degradation with `/api/health` still HTTP 200, `noindex`, desktop, keyboard, and 320 px behavior in `tests/e2e/bank-movements.spec.ts`
...
- [ ] T060 Validate additive migration, previous-image compatibility, logical backup/restore boundaries, retention-floor behavior, and stale-cursor forward recovery against a disposable database by following `specs/20260916-holded-bank-movements/quickstart.md`
- [ ] T061 Run Prisma validation/generation, focused tests, lint, typecheck, full tests and coverage, production dependency audit, production build, Playwright E2E, and every repeatable manual scenario from `specs/20260916-holded-bank-movements/quickstart.md`, recording the SC-006 result below 60 seconds
