...
This feature widens the application's exposure considerably: it holds identifying data about people
who never registered, and it integrates with an accounting system and a bank.

### Personal data held

| Data | Source | Subject |
|---|---|---|
| Name, tax identifier, address, phone, email | Gravity Forms submission | The requester |
| Payer name and reference | Bank transactions | Whoever sent the transfer, not necessarily the requester |
| Stay dates, headcount, board type | Submission | The group |

Requesters have no account and never consented to registration, so the lawful basis is the
pre-contractual step they initiated. Bank transactions are the sharpest edge: they can reveal third
parties unconnected to any booking. Only the fields needed for matching are stored, and unmatched
credits are the shortest-lived records in the system.
