...
| New scheduler inside the `app` container | Intake, outbox drain and expiry are scheduled and retryable | A separate `worker` container is the textbook separation, but it needs a bundler with a platform-specific native binary to resolve the `@/` aliases, which is real friction on an ARM64 Raspberry Pi build. The outbox claims jobs atomically and intake is idempotent per entry id, so running two instances is safe by construction rather than by assumption; the scheduler is behind `BOOKING_SCHEDULER`, so it can be moved out later without touching the domain code |
